...
* @Last Modified time: 2014-07-24 14:53:11
*/
?>
<?php
/**
...
class HTML2PDF_locale
{
    ...
    static public function load($code)
    {
        ...
        while (!feof($handle)) {
            $line = fgetcsv($handle);
//            $pkCount = (is_array($this->documents) ? count($this->documents) : 0);// meti esta
//            if (count($line)!=2) continue;
            $test=(is_array($line) ? count($line) : 0); //meti esta
            if($test !=2 ) continue; //meti esta
