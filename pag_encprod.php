...
<?php
/*
  Created on : 31/Ago/2013, 17:02:40
  Author     : Ricardo Órfão
 */
...
$query_produto = $connection->prepare("SELECT s.id AS id_produto, f.id AS id_fornecedor, f.nome_abrev, s.nome, @preco_un := preco AS preco_un, @taxa := (SELECT rp.valor FROM produto p INNER JOIN regra_produto rp ON p.id=rp.id_produto INNER JOIN regra r ON rp.id_regra=r.id_regra WHERE p.id=s.id ORDER BY data DESC LIMIT 1) AS taxa, (@preco_un * (1 + @taxa/100)) AS total, descricao FROM empresa emp INNER JOIN atividade a ON emp.atividade=a.id INNER JOIN atividade_familia af ON af.id_atividade=a.id INNER JOIN familia fa1 ON af.id_familia=fa1.id INNER JOIN familia fa2 ON fa1.id=fa2.parent INNER JOIN familia fa3 ON fa2.id=fa3.parent INNER JOIN produto s ON fa3.id=s.familia INNER JOIN fp_stock fp ON fp.id_produto=s.id INNER JOIN fornecedor f ON fp.id_fornecedor=f.id WHERE emp.ativo='1' AND emp.id_empresa=:id_empresa ORDER BY s.nome");
$query_produto->execute(array(':id_empresa' => $_SESSION['id_empresa']));
$num_produto = $query_produto->rowCount();

$query_produto_add = $connection->prepare("SELECT c.id AS id_item_add, p.id AS id_produto, f.id AS id_fornecedor, f.nome_abrev AS nome_fornecedor, p.nome, c.preco, c.quantidade, c.iva AS taxa, c.valor FROM carrinho c INNER JOIN produto p ON p.id=c.item_adicionado INNER JOIN fornecedor f ON f.id=c.id_fornecedor WHERE id_empresa=:id_empresa ORDER BY c.id");
                        ...
                        <table id="tblProdutos" name="tblProdutos" class="tabela">
                            ...
                            <?php while ($linha_itens = $query_produto->fetch(PDO::FETCH_ASSOC)) { ?>
                                <tr class="tbody">
                                    ...
                                    <td class="preco" style="padding: 1%;"><?php echo number_format($linha_itens['preco_un'], 2, ',', '.'); ?> <?php echo $linha_moeda['simbolo']; ?></td>
                                    <td style="padding: 1%;"><?php echo number_format($linha_itens['taxa'], 0, ',', '.'); ?>%</td>
                            ...
                            <table id="tblCarrinho" name="tblCarrinho" style="width: 100%;">
                                <?php while ($linha_produto_add = $query_produto_add->fetch(PDO::FETCH_ASSOC)) { ?>
                                    <tr>
                                        ...
                                        <td style="width: 35%; text-align: right; padding-right: 5px;"><?php echo number_format($linha_produto_add['preco'] * $linha_produto_add['quantidade'] + ($linha_produto_add['preco'] * $linha_produto_add['quantidade'] * ($linha_produto_add['taxa'] / 100)), 2, ',', '.'); ?> <?php echo $linha_moeda['simbolo']; ?></td>
                                    </tr>
                                <?php } ?>
                            </table>
